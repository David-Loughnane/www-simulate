...
, 'motive': {
    title: 'Motivation'
  , description:
  'Some companies including Netflix and YouTube already make use of this idea and locate popular content closer to the locations where there is a higher demand for that specific content. However, they are still doing that in a location based way, by prepopulating the servers with popular content in order to reduce traffic during peek times. We wanted to implement popularity based caching on the protocol level and develop an algorithm that would allow the caching nodes in a network decide what content to cache based on that contents popularity score. Data objects in such a network are addressed by their content using a hash function and not by the location of the server. Such networks are called "Content-Centric" and over the past years this area has attracted a lot of research interest'
  }
, 'cache': {
    title: 'Caching'
  , description: 
  'Our caching algorithms are all based on the same concept by using a populrity table for keeping track of the popularity for specific content that is traveling throughout the network. All caching nodes maintain a popularity table and every time a data object travels through them it increments the popularity of that object by one in its table. As soon as the content reaches an established threshold, it is being cached in the node. In order to avoid caching content that becomes unpopular over time, the popularity of all contents is being decremend every time interval by a fixed amount. As soon as the cache becomes full, the content with the lowest accumulated popularity score is being deleted and replaced by the new popular content. If a specific content had not been requested for a long period of time, it is being deleted as well.'
  }
, 'algo': {
    title: 'Algorithms'
...
, 'who': {
    title: 'Team'
  , description: 'Supervisor: Kin Leung (Department of Electrical and Electronic Engineering <br> Group Members: William F., Noppawee A., Hugh O., Lisa C., David L., Wing L.'
  }
}
